...
/**
 * The gulp task compiles and minifies scss/mailutils.scss file into css/mailutils.min.css.
 * Also prefix the output css selector with `#bootstrap-theme` selector except the output.
 * selector starts from either `body`, `page-civicrm-mailutils` or `.___outside-namespace` classes.
 *
 * @returns {object} stream
 */
function sassTask () {
  return gulp.src('scss/mailutils.scss')
    .pipe(glob())
    ...
    .pipe(postcss([postcssPrefix({
      ...
      exclude: [/^body/, /page-civicrm-mailutils/, OUTSIDE_NAMESPACE_REGEX]
