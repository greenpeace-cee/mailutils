...
function mailutils_civicrm_tabset($tabsetName, &$tabs, $context) {
  if ($tabsetName != 'civicrm/contact/view') {
    return;
  }

  if (!CRM_Core_Permission::check('access mailutils')) {
    return;
  }

  $url = CRM_Utils_System::url(
    'civicrm/mailutils/contact/thread',
    "cid={$context['contact_id']}"
  );
  // TODO: move to utility
  $thread_count = 0;
  if (!empty($context['contact_id'])) {
    $thread_count = \Civi\Api4\MailutilsThread::get()
      ->addWhere('involved_contact_id', '=', $context['contact_id'])
      ->execute()
      ->count();
  }
  $tabs[] = [
    'id'     => 'mailutils_contact_thread',
    'url'    => $url,
    'title'  => 'Emails',
    'weight' => 71,
